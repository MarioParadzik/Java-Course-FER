...
public class LinkedListIndexedCollection<T> implements List<T>{
	...
	public void insert(T value, int position) {
		...
		if (last == first){
			...
			for (int i = 0; i < this.size; i++) {
				if(position == i) {
					ListNode<T> insertNode = new ListNode<T>(value);
	...
	public Object[] toArray() {
		...
		int i = 0;
		for (ListNode<T> pom = first; pom != null; pom = pom.next) {
			polje[i++] = pom.value;
		}
		return polje;
	}
	 ...
	 */
	private static class LLIC<T> implements ElementsGetter<T> {
		 ...
		 */
		public LLIC(LinkedListIndexedCollection<T> collection, long modificationCount) {
	...
	public ElementsGetter<T> createElementsGetter() {
		return new LLIC<T>(this, modificationCount);
	}
